...
export function EventCard({ event, priority = false }: { event: Event; priority?: boolean }) {
  const label = `Reserve ${event.title}`;
  ...
  return (
    <article className="event-card group flex cursor-pointer flex-col rounded-[28px] p-2">
      ...
      {event.registrationUrl ? (
        <a href={event.registrationUrl} className={hitClassName} aria-label={label} />
      ) : (
        <button type="button" className={hitClassName} aria-label={label} />
      )}
    </article>
  );
}
